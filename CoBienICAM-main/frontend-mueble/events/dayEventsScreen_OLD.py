...
class DayEventsScreen(Screen):
    ...
    current_location = StringProperty("Bilbao")
    ...
    def voice_add(self):
        if not self.current_day:
            return
        title = self._listen_spanish(_("Di el título del evento personal"))
        if not title:
            self._speak(_("No he entendido el título."))
            return
        desc = self._listen_spanish(_("Di la descripción del evento")) or _("Sin descripción")
        ...
        if ok:
            self._speak(_("Evento añadido."))
            if hasattr(self.store, "reload"):
                self.store.reload()
            self._build_list()
            self._notify_refresh()
        else:
            self._speak(_("Ha ocurrido un error al añadir el evento."))
    ...
    def _delete_event(self, event_id: str):
        ...
        if ok:
            if hasattr(self.store, "reload"):
                self.store.reload()
            self._build_list()
            self._speak(_("Evento eliminado."))
            self._notify_refresh()
        else:
            self._speak(_("No he podido eliminar el evento."))
