...
# int16 event codes as defined in conversion.json / proximity/update payload
EVENT_MOTION_START = 0x5EBA
EVENT_APPROACH = 0xD157
EVENT_MOTION_END = 0xE5AB
...
def log_proximity_event(can_id: int, event_code: int) -> None:
    ...
    if log_label is not None:
        write_log_txt(
            source="proximity", target=f"{log_label} {LABEL_MAP[position]}"
        )
